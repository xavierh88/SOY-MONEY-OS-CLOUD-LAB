...
// These values are intentionally local and fake. The app only creates a lazy
// pg Pool at import time; every request in this file avoids database branches.
...
test("health and Zod response contracts are aligned", async () => {
  ...
  const validInput = {
    ...
    description: "A real schema fixture, never sent to a provider",
    sector: "software",
    problem: "Contract drift",
    targetCustomer: "API maintainers",
    proposedSolution: "Run offline checks",
...
test("owner authentication isolates the single configured owner", async () => {
  ...
  assertStableError(unauthenticated.body);

  const otherOwner = await request("/api/dashboard", { headers: ownerHeaders("another-user") });
  assert.equal(otherOwner.response.status, 403);
  assertStableError(otherOwner.body);
  ...
  assert.equal(unprovisioned.response.status, 503);
  assertStableError(unprovisioned.body);
});

test("representative principal routes expose stable API errors offline", async () => {
  // Malformed path parameters are rejected before any storage access.
  const invalidPath = await request("/api/opportunities/not-an-integer", {
    headers: ownerHeaders(),
  });
  assert.equal(invalidPath.response.status, 400);
  assertStableError(invalidPath.body);

  const unknownPath = await request("/api/does-not-exist", {
    headers: ownerHeaders(),
  });
  assert.equal(unknownPath.response.status, 404);
  assertStableError(unknownPath.body);

  const invalidService = await request("/api/service/v1/callback", {
    method: "POST",
    body: {},
    headers: { "x-service-operation": "windmill.callback" },
  });
  assert.equal(invalidService.response.status, 403);
  assertStableError(invalidService.body);

  const expiredService = await request("/api/service/v1/callback", {
    method: "POST",
    body: {},
    headers: {
      "x-service-id": "windmill",
      "x-service-operation": "windmill.callback",
      "x-dispatch-id": "offline-dispatch",
      "x-service-timestamp": "1",
    },
  });
  assert.equal(expiredService.response.status, 401);
  assertStableError(expiredService.body);

  const body = JSON.stringify({ dispatch_id: "offline-dispatch" });
  const bodyHash = createHash("sha256").update(body).digest("hex");
  const unavailableService = await request("/api/service/v1/callback", {
    method: "POST",
    ...
    headers: {
      ...
      "x-service-operation": "windmill.callback",
      "x-dispatch-id": "offline-dispatch",
      "x-service-timestamp": String(Math.floor(Date.now() / 1000)),
      "x-body-sha256": bodyHash,
      "x-service-signature": "offline-signature",
    },
  });
  assert.equal(unavailableService.response.status, 503);
  assertStableError(unavailableService.body);
});
...
test("OpenAPI, generated client, and Zod exports cover principal routes", () => {
  ...
  // Human operations are owner-gated in the source spec and client generation
  // must not invent a caller-controlled owner identifier.
...
test("client errors preserve stable payloads for the complete HTTP status matrix", async () => {
  const { ApiError, customFetch, setAuthTokenGetter, setBaseUrl } =
    await import("../../../lib/api-client-react/src/custom-fetch.ts");
  ...
  try {
    ...
    globalThis.fetch = async (input, init) => {
      captured = { input, init };
      return new Response(JSON.stringify({ error: `status-${init?.method ?? "GET"}` }), {
