...
import { ArrowLeft, CheckCircle2, CircleAlert, FileSearch, ActivityIcon, ShieldAlert, Play, RefreshCw, Download, ExternalLink } from 'lucide-react';
...
export default function ProyectoDetailPage() {
  ...
  function actionForStage() {
    ...
    if (stage === 'SELL_READY' || stage === 'WAITING_HUMAN') return <div className="flex flex-col items-end gap-2"><div className="text-xs text-muted-foreground max-w-sm">Checkpoint humano activo. No se publica, vende ni registra resultado REAL desde esta pantalla. Completa primero la acción humana correspondiente.</div><Link href="/acciones" className="button button-secondary"><ExternalLink size={14} /> ABRIR COLA HUMANA</Link></div>;
  ...
  function downloadableFiles(data: unknown) {
    if (!data || typeof data !== 'object') return [] as Array<{ path: string; sha256?: string; bytes?: number }>;
    const value = data as Record<string, unknown>;
    const nested = value.artifactManifest && typeof value.artifactManifest === 'object' ? value.artifactManifest as Record<string, unknown> : undefined;
    const files = Array.isArray(value.files) ? value.files : Array.isArray(nested?.files) ? nested.files : [];
    return files.flatMap((file) => {
      if (!file || typeof file !== 'object') return [];
      const entry = file as Record<string, unknown>;
      if (typeof entry.path !== 'string' || !entry.path || entry.path.startsWith('/') || entry.path.split('/').some((part) => !part || part === '..')) return [];
      return [{ path: entry.path, sha256: typeof entry.sha256 === 'string' ? entry.sha256 : undefined, bytes: typeof entry.bytes === 'number' ? entry.bytes : undefined }];
    });
  }

  function artifactDownloadHref(filePath: string) {
    return `/api/projects/${id}/artifacts/${filePath.split('/').map(encodeURIComponent).join('/')}`;
  }
    ...
    <DataState loading={project.isLoading} error={!!project.error} empty={!project.isLoading && !d} onRetry={() => void project.refetch()}>
      {d && <>
        ...
        <div className="mt-8 detail-grid"><div className="detail-primary">
          ...
          <section className="panel"><div className="panel-heading"><div><div className="eyebrow">Entregables</div><h3>Artefactos del Proyecto (Manifests)</h3></div></div>{d.artifacts.length > 0 ? <div className="flex flex-col gap-4">{d.artifacts.map((a, i) => { const files = downloadableFiles(a.data); return <div key={i} className="p-3 border border-border rounded bg-card"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><FileSearch size={16} className="text-accent" /><strong className="text-sm font-mono text-accent">{a.kind}</strong></div><span className="text-[10px] bg-secondary px-2 py-1 rounded font-mono text-muted-foreground">REF: {a.sourceId.substring(0, 16)}</span></div>{files.length > 0 && <div className="mb-3 grid gap-2">{files.map((file) => <div key={file.path} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border p-2"><div className="min-w-0"><div className="text-xs font-mono break-all">{file.path}</div><div className="text-[10px] text-muted-foreground">{file.bytes !== undefined ? `${file.bytes} bytes` : 'Tamaño en manifest no disponible'}{file.sha256 ? ` · SHA256 ${file.sha256.slice(0, 12)}…` : ''}</div></div><a className="button button-secondary" href={artifactDownloadHref(file.path)}><Download size={14} /> DESCARGAR</a></div>)}</div>}<div className="bg-secondary/50 rounded p-3 overflow-x-auto">{Object.entries(a.data as any).map(([k, v]) => <div key={k} className="mb-2 last:mb-0"><span className="text-[10px] text-muted-foreground uppercase">{k}</span>{typeof v === 'string' ? <div className="text-xs font-mono mt-0.5 break-all whitespace-pre-wrap">{v}</div> : <pre className="text-xs font-mono mt-0.5 text-muted-foreground whitespace-pre-wrap break-all">{JSON.stringify(v, null, 2)}</pre>}</div>)}</div></div>; })}</div> : <p className="text-sm text-muted-foreground">Sin artefactos generados aún.</p>}</section>
