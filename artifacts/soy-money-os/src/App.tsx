...
const navGroups = [
  {
    ...
    items: [
      ...
      { href: '/operaciones', label: 'Centro de operaciones', icon: TerminalSquare },
...
function CycleControl({ onCreated }: { onCreated?: () => void }) {
  ...
  const { data: cycle } = useGetCurrentCycle({ query: { queryKey: getGetCurrentCycleQueryKey(), refetchInterval: (query) => { const state = query.state.data?.state; if (state === 'COMPLETED' || state === 'FAILED' || state === 'REJECTED') return false; return 3000; } } });
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (query.trim().length < 2) return; const idempotencyKey = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2); startCycle.mutate({ data: { query: query.trim(), idempotencyKey } }, { onSuccess: () => { setQuery(''); queryClient.invalidateQueries({ queryKey: getGetCurrentCycleQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetControlTowerOverviewQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetControlTowerTimelineQueryKey() }); onCreated?.(); } }); };
  const handleDecision = (decision: 'approved' | 'rejected') => { if (!cycle?.id) return; decideCycle.mutate({ id: cycle.id, data: { decision } }, { onSuccess: () => { queryClient.invalidateQueries({ queryKey: getGetCurrentCycleQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); queryClient.invalidateQueries({ queryKey: getListApprovalsQueryKey() }); queryClient.invalidateQueries({ queryKey: getListOpportunitiesQueryKey() }); queryClient.invalidateQueries({ queryKey: getListProjectsQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetControlTowerOverviewQueryKey() }); queryClient.invalidateQueries({ queryKey: getGetControlTowerTimelineQueryKey() }); } }); };
  const stages = ['DISCOVERY', 'RESEARCH', 'EVIDENCE', 'VALIDATION', 'WAITING_APPROVAL', 'APPROVED', 'BUILD', 'QA', 'SELL_READY', 'RESULT', 'LEARNING', 'COMPLETED'];
  return <div className="launch-panel cycle-panel"><div className="launch-intro"><div className="launch-icon"><Zap size={20} /></div><div><div className="eyebrow">Motor Autónomo</div><h2>Operación de ciclo completo</h2><p>Describe una tensión de mercado. El motor investigará, pedirá tu decisión y preparará el proyecto sin publicar ni vender.</p></div></div><form onSubmit={submit} className="launch-form"><div className="launch-input-wrap"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ej. software de cobro para clínicas" disabled={!!(isActive || startCycle.isPending)} data-testid="input-cycle-query" /><span>⌘ ↵</span></div><button type="submit" className="button button-primary" disabled={!!(startCycle.isPending || query.trim().length < 2 || isActive)} data-testid="button-start-cycle">{startCycle.isPending ? <RefreshCw size={15} className="spin" /> : <Play size={15} />}{startCycle.isPending ? 'Iniciando' : 'INICIAR CICLO'}</button></form>{cycle && <div className="cycle-active-section animate-enter"><div className="eyebrow">CICLO ACTUAL: {cycle.state}</div><div className="cycle-stages">{stages.map((s) => { const isCurrent = cycle.stage === s && !isTerminal; const isDone = stages.indexOf(cycle.stage) > stages.indexOf(s) || isTerminal; return <div key={s} className={cx('cycle-stage', isCurrent && 'stage-current', isDone && 'stage-done')}>{s.replaceAll('_', ' ')}</div>; })}</div><div className="cycle-meta"><div><strong>ID:</strong> CYC-{String(cycle.id).padStart(4, '0')}</div><div><strong>Inicio:</strong> {formatTime(cycle.startedAt)}</div><div><strong>Actualización:</strong> {formatTime(cycle.updatedAt)}</div>{cycle.opportunityId && <div><strong>Oportunidad:</strong> #{cycle.opportunityId}</div>}{cycle.projectId && <div><strong>Proyecto:</strong> #{cycle.projectId}</div>}{cycle.approvalId && <div><strong>Aprobación:</strong> #{cycle.approvalId}</div>}</div>{(opportunity.data || cycleApproval || project.data) && <div className="cycle-detail">{opportunity.data && <div><strong>{opportunity.data.name}</strong><span>{opportunity.data.status} · {opportunity.data.proofStatus} · {opportunity.data.evidence.length} evidencias</span></div>}{cycleApproval && <div><strong>Decisión humana: {cycleApproval.status}</strong><span>{cycleApproval.reason}</span></div>}{project.data && <div><strong>Proyecto: {project.data.project.status}</strong><span>QA {project.data.project.qaStatus ?? 'pendiente'} · Paquete comercial {project.data.project.sellPackage ? 'preparado' : 'pendiente'} · Resultado {project.data.result?.status ?? 'pendiente'} · Aprendizaje {project.data.learning?.status ?? 'pendiente'}</span></div>}</div>}{cycle.state === 'COMPLETED' ? <div className="cycle-message" style={{ borderColor: 'hsl(145 43% 41%)', backgroundColor: 'hsl(145 43% 41% / 0.1)' }}>Proyecto preparado. No se ha realizado una venta real.</div> : cycle.message ? <div className="cycle-message"><strong>{cycle.stage}:</strong> {cycle.message}</div> : null}{cycle.error && <div className="cycle-error"><strong>Error{cycle.errorService ? ` (${cycle.errorService} ${cycle.errorStatusCode})` : ''}:</strong> {cycle.error}</div>}{cycle.stage === 'WAITING_APPROVAL' && !isTerminal && <div className="cycle-actions"><button type="button" className="button button-secondary" onClick={() => handleDecision('approved')} disabled={decideCycle.isPending} data-testid="button-approve-cycle">{decideCycle.isPending ? 'Procesando...' : <><Check size={14} /> APROBAR</>}</button><button type="button" className="button" style={{ backgroundColor: 'hsl(var(--destructive))', color: 'hsl(var(--destructive-foreground))', borderColor: 'transparent' }} onClick={() => handleDecision('rejected')} disabled={decideCycle.isPending} data-testid="button-reject-cycle">{decideCycle.isPending ? 'Procesando...' : <><X size={14} /> RECHAZAR</>}</button></div>}</div>}</div>;
}
...
function E2EFixtureSignIn({ onSignIn, signUp = false }: { onSignIn: () => void; signUp?: boolean }) { return <div className="flex min-h-[100dvh] items-center justify-center bg-background px-4"><section className="panel w-full max-w-md p-8"><div className="eyebrow">E2E fixture · sin Clerk</div><h1 className="mt-3">{signUp ? 'Crear cuenta' : 'Iniciar sesión'}</h1><p className="mt-4 text-muted-foreground">Identidad determinista para pruebas de navegador. No usa credenciales humanas ni servicios externos.</p><button className="button button-primary mt-8 w-full" onClick={onSignIn} data-testid="button-e2e-sign-in">{signUp ? 'Crear cuenta de prueba' : 'Entrar como propietario de prueba'}</button></section></div>; }
function E2EFixtureRoutes() { const [signedIn, setSignedIn] = useState(() => window.sessionStorage.getItem('soy-money-e2e-auth') === 'signed-in'); const signIn = () => { window.sessionStorage.setItem('soy-money-e2e-auth', 'signed-in'); setSignedIn(true); }; const signOut = () => { window.sessionStorage.removeItem('soy-money-e2e-auth'); setSignedIn(false); }; return <QueryClientProvider client={queryClient}><Switch><Route path="/" component={() => signedIn ? <Redirect to="/user-portal" /> : <HomePage />} /><Route path="/sign-in/*?" component={() => signedIn ? <Redirect to="/user-portal" /> : <E2EFixtureSignIn onSignIn={signIn} />} /><Route path="/sign-up/*?" component={() => signedIn ? <Redirect to="/user-portal" /> : <E2EFixtureSignIn onSignIn={signIn} signUp />} /><Route component={() => signedIn ? <ExistingAppRouter /> : <Redirect to="/" />} /></Switch><button type="button" className="sr-only" onClick={signOut} data-testid="button-e2e-sign-out">Sign out fixture</button><TooltipProvider><Toaster /></TooltipProvider></QueryClientProvider>; }
function App() { return <WouterRouter base={basePath}>{e2eFixtureMode ? <E2EFixtureRoutes /> : <ClerkProviderWithRoutes />}</WouterRouter>; }
export default App;
